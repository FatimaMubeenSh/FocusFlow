    ...
    <div className="flex flex-col gap-2">
      {isEditing ? (
        <div className={`bg-white text-gray-800 rounded-md shadow px-4 mb-2 py-3 ${editedType === "key" ? "" : "border border-gray-200"}`}>
            ...
            <select
              value={editedType}
              onChange={(e) => setEditedType(e.target.value as "key" | "secondary")}
              className="border border-gray-300 rounded px-2 py-1"
            >
          ...
          <div className="flex gap-2 mt-2 justify-end">
            <button onClick={handleSave} className="bg-blue-500 text-white rounded px-2 py-1">
              Save
            </button>
            <button onClick={() => setIsEditing(false)} className="bg-gray-300 text-black rounded px-2 py-1">
              Cancel
            </button>
          </div>
        </div>
        ...
        <div className={`bg-white text-gray-800 rounded-md shadow px-4 mb-2 py-3 ${task.type === "key" ? "" : "border border-gray-200"}`}>
          ...
          <div className="flex gap-2 mt-2 justify-end">
            <Pencil size={18} className="text-blue-500 cursor-pointer" onClick={() => setIsEditing(true)} />
